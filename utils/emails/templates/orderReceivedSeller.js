// utils/emails/templates/orderReceivedSeller.js
const baseLayout = require("../layouts/baseLayout");

const orderReceivedSellerTemplate = ({
  sellerName,
  orderId,
  productName,
  qty,
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 24px; font-size: 16px; color: #2a2a2e; line-height: 1.6;">
      ...
      Great news! You’ve received a new order on <strong>Semamart</strong>. 
      Please review the details below and start processing it to ensure timely fulfillment.
    </p>
    ...
    <p style="margin: 32px 0 0; font-size: 14px; color: #9ca3af; line-height: 1.5;">
      Timely processing helps ensure faster delivery and better seller ratings. 
      Need help? <a href="${frontendUrl}/support" style="color: #6EC1E5; text-decoration: underline;">Contact Seller Support</a>.
    </p>
  `;

  return baseLayout({
    title: "New Order Received | Semamart",
    preheader: "New Order Alert! You have received a new order #" + orderId,
    bodyHtml,
  });
};

module.exports = orderReceivedSellerTemplate;